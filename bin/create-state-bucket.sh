...
create_bucket() {
  ...
  aws s3api create-bucket --bucket "$BUCKET_NAME" --region "$REGION" --create-bucket-configuration "LocationConstraint=$REGION" --acl private &&
    ...
    aws s3api put-bucket-encryption --bucket "$BUCKET_NAME" --region "$REGION" --server-side-encryption-configuration "$bucket_config" &&
    log_success "${bold}Created bucket '${BUCKET_NAME}' in region '${REGION}'."
}
...
write_state_tf() {
  need_vars DEST BUCKET_NAME REGION || exit 1
  cat >"${DEST}" <<EOF
terraform {
  backend "s3" {
    bucket         = "${BUCKET_NAME}"
...
main() {
  ...
  if bucket_exists "$BUCKET_NAME"; then
    ...
    # shellcheck disable=SC2154
    log_notice "${bold}Creating bucket: ${underline}$BUCKET_NAME${underlineoff} in region $REGION"
